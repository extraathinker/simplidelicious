...
@section('main-container')
    ...
    <div class="container-fluid" style="min-height: 500px;">
        <div class="container p-3">
            <div class="row p-2">
				<div class="col-lg-8">
					<form method="get" action="/admin/searchapproval">
            ...
            <table class="table border table-danger table-striped">
                ...
                @foreach($recipes as $recipe)
                    <tr>
                        ...
                        <td>{{$recipe->approvedstatus}}</td>
                        <td>{{$recipe->Date}}</td>
                        <td>{{$recipe->image}}</td>
                        <td>
                            @if (!is_null($recipe->approvedstatus))
                                @if ($recipe->approvedstatus == 0)
                                    <a class="btn btn-primary me-3" href="/admin/approve/{{$recipe->approvalId}}">
                                        Approve
                                    </a>
                                @else
                                    <a class="btn btn-primary me-3" href="/admin/disapprove/{{$recipe->approvalId}}">
                                        Disapprove
                                    </a>
                                @endif
                            @endif
                            <a class="btn btn-danger ms-3" href="{{url('/')}}/admin/deleteapproval/{{$recipe->approvalId}}">Delete</a></td>
                    </tr>
                @endforeach
